...
check(pl.title === "Yangi so'rov" && pl.body.includes(email) && pl.count === pending, `payload has email and pending count (${pl.count})`);
...
const pendingNow = (await q(`select count(*)::int n from user_access where status='pending'`))[0].n;
check((await page.evaluate(() => window.__badge.at(-1))) === pendingNow, `app icon badge = pending count (${pendingNow})`);
...
await cdp.send('ServiceWorker.deliverPushMessage', { origin: ADMIN.replace(/\/$/, ''), registrationId: reg.registrationId, data: JSON.stringify({ title: "Yangi so'rov", body: `${email} ro'yxatdan o'tdi`, count: 4, tag: 'requests' }) });
await sleep(800);
const notes = await page.evaluate(async () => (await (await navigator.serviceWorker.ready).getNotifications()).map(n => ({ title: n.title, body: n.body, tag: n.tag })));
check(notes.some(n => n.title === "Yangi so'rov" && n.body.includes(email) && n.tag === 'requests'), 'SW shows notification on push (app closed or open)');
